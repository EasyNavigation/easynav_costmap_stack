...
#include "easynav_costmap_serest_controller/SerestController.hpp"
...
SerestController::on_initialize()
{
  ...
  // Estimación desde costmap
  node->declare_parameter<int>(ns + ".lethal_cost_threshold", lethal_cost_threshold_);
  ...
  node->get_parameter<int>(ns + ".lethal_cost_threshold", lethal_cost_threshold_);
...
SerestController::closest_obstacle_distance(
  const NavState & nav_state, const Costmap2D & map, const Vec2 & robot_xy) const
{
  ...
  // 2) Estimación simple desde el costmap (ventana cuadrada de radio dist_search_radius_)
  // API asumida similar a costmap_2d: worldToMap, getCost, getResolution, ...
  unsigned int mx_center{0}, my_center{0};
  if (!map.worldToMap(robot_xy.x, robot_xy.y, mx_center, my_center)) {
    return std::numeric_limits<double>::infinity();
  }

  const double res = map.getResolution();
  const int rad_cells = static_cast<int>(std::ceil(dist_search_radius_ / res));
  double best = std::numeric_limits<double>::infinity();

  const unsigned int size_x = map.getSizeInCellsX();
  const unsigned int size_y = map.getSizeInCellsY();

  for (int dy = -rad_cells; dy <= rad_cells; ++dy) {
    int cy = static_cast<int>(my_center) + dy;
    if (cy < 0 || cy >= static_cast<int>(size_y)) {continue;}
    for (int dx = -rad_cells; dx <= rad_cells; ++dx) {
      int cx = static_cast<int>(mx_center) + dx;
      if (cx < 0 || cx >= static_cast<int>(size_x)) {continue;}

      unsigned char c = map.getCost(cx, cy);
      if (c >= lethal_cost_threshold_) {
        // Distancia euclídea centro de celda -> robot
        double wx = 0.0, wy = 0.0;
        map.mapToWorld(static_cast<unsigned int>(cx), static_cast<unsigned int>(cy), wx, wy);
        double d = std::hypot(wx - robot_xy.x, wy - robot_xy.y);
        if (d < best) {best = d;}
      }
    }
  }
...
SerestController::update_rt(NavState & nav_state)
{
  ...
  const double d_closest = closest_obstacle_distance(nav_state, map, robot_xy);
